...
struct CodesignChecker {
  ...
  let kSecCSCustomFlags = SecCSFlags.init(
    rawValue: kSecCSDoNotValidateResources | kSecCSCheckNestedCode)
  ...
  private func prepareSelf() throws -> SecStaticCode? {
    ...
    guard resultCode == errSecSuccess, let secCode = secCodeSelf else {
      ...
      throw CodesignCheckerError(
        type: .secCodeCopySelf,
        description: String(describing: SecCopyErrorMessageString(resultCode, nil)),
        methodName: #function, fileName: #file, lineNumber: #line)
    }
  ...
  private func prepare(withPID pid: pid_t) throws -> SecStaticCode? {
    ...
    var resultCode = SecCodeCopyGuestWithAttributes(
      nil,
      kSecAttributes as CFDictionary,
      kSecCSDefaultFlags, &secCodePID)

    guard resultCode == errSecSuccess, let secCode = secCodePID else {

      throw CodesignCheckerError(
        type: .secCodeCopyGuestWithAttributes,
        description: String(describing: SecCopyErrorMessageString(resultCode, nil)),
        methodName: #function, fileName: #file, lineNumber: #line)
    }
  ...
  private func prepare(withURL url: URL) throws -> SecStaticCode? {
    ...
    guard resultCode == errSecSuccess && secStaticCode != nil else {
      ...
      throw CodesignCheckerError(
        type: .secStaticCodeCreateWithPath,
        description: String(describing: SecCopyErrorMessageString(resultCode, nil)),
        methodName: #function, fileName: #file, lineNumber: #line)
    }
  ...
  private func getCertificates(secStaticCode: SecStaticCode) throws -> [SecCertificate] {
    ...
    let resultCode = SecCodeCopySigningInformation(
      secStaticCode,
      SecCSFlags(rawValue: kSecCSSigningInformation), &secCodeInfoCFDict)

    guard resultCode == errSecSuccess, let secCodeInfo = secCodeInfoCFDict as? [String: Any] else {

      throw CodesignCheckerError(
        type: .secCodeCopySigningInformation,
        description:
          String(
            describing:
              SecCopyErrorMessageString(resultCode, nil)),
        methodName: #function, fileName: #file, lineNumber: #line)
    }

    guard let secCertificates = secCodeInfo[kSecCodeInfoCertificates as String] as? [SecCertificate]
    else {

      throw CodesignCheckerError(
        type: .genericError,
        description: "Failed to obtain certificates from the information dictionary",
        methodName: #function, fileName: #file, lineNumber: #line)
    }
  ...
  public func getCertificatesSelf() throws -> [SecCertificate] {
    ...
    do {
      ...
      if !isValid(secStaticCode: secStaticCode!) {
        throw CodesignCheckerError(
          type: .genericError, description: "Validation failed", methodName: #function,
